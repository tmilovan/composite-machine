# STEP as Pure Composite Expression — Indicator Polynomial Test

<aside>
🎯

**Goal:** Express the BB2 STEP function as a pure composite expression using indicator polynomials. No Python `if`, no `dict`, no branching — only `+`, `×`, `read_dim`, `ZERO`, `INF`.

**Status:** ✅ PASSED — Open Question #1 from Self-Hosted Execution: ANSWERED.
...
For a 2-state, 2-symbol TM, there are 4 possible (state, symbol) pairs. Each pair maps to a fixed action (write, move, new_state). If we can build **indicator polynomials** that equal 1 for exactly one pair and 0 for the others, then STEP is just a weighted sum:

```
STEP(config) = I(0,0) × action₀₀ + I(0,1) × action₀₁ + I(1,0) × action₁₀ + I(1,1) × action₁₁
```

The key operations:

- **×ZERO** = move head right (shift tape left)
- **×INF** = move head left (shift tape right)
- **st(tape)** = read symbol at head (dim 0)
- **tape + (new - old)** = write symbol at head

All composite-native. All algebraic.
...
## Key Insight: Branching Without Branching

The indicator polynomial trick is simple but profound:

```
I(state=0, symbol=0) = (1 - state) * (1 - symbol)
```

When `state=0` and `symbol=0`, this equals 1. For any other combination, it equals 0. **This is branching via arithmetic** — no `if`, no pattern matching, no case analysis. Just multiplication.

The four indicators are **mutually exclusive and exhaustive**: exactly one equals 1, the rest equal 0, and they sum to 1. So the weighted sum:

```
new_tape = I₀₀ * action₀₀ + I₀₁ * action₀₁ + I₁₀ * action₁₀ + I₁₁ * action₁₁
```

selects exactly one action — without ever testing a condition.

---

## Why ×ZERO and ×INF Are Perfect

| **Operation** | **Composite expression** | **What it does** |
| --- | --- | --- |
| Move head right | `tape × ZERO` | Shifts all dims down by 1. New dim 0 = old dim 1. |
| Move head left | `tape × INF` | Shifts all dims up by 1. New dim 0 = old dim -1. |
| Stay | `tape` | Identity. No shift. |
| Round-trip | `tape × ZERO × INF` | = tape. Reversible! |

Head movement IS multiplication. This works because ZERO = |1|₋₁ and INF = |1|₁ are single-term composites, so multiplication by them is a pure dimensional shift.
