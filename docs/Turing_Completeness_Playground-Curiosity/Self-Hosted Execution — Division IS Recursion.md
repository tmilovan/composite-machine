# Self-Hosted Execution — Division IS Recursion

<aside>
🧪

**Experiment:** Can the entire busy beaver computation be expressed as a single algebraic operation (division) in the composite algebra, with no external loop?

**Hypothesis:** `poly_divide(config₀, I - STEP)` = the full computation.

**Status:** Experimental — testing in stages.
...
1. **Coefficient extraction is limits** — built into the system, not external
2. **Division generates recursion** — `1/(1-x) = 1 + x + x² + ...` is infinite iteration expressed algebraically
3. **Therefore:** if STEP is a composite operation, then `1/(1-STEP)` applied to the initial config IS the computation

The Python `while` loop that drives the TM is structurally identical to the `for` loop inside `_poly_divide`. Both peel off one term per iteration and feed the remainder back. If `_poly_divide` is "native algebra," then so is the TM execution.

---

## Stage 1: Config as One Composite

Pack the entire TM state (tape + state + head) into a single Composite number.
...
| --- | --- | --- |
| **1** | Tape + state + head = one Composite | The TM configuration IS a composite number |
| **2** | One step is Composite → Composite | No Python dict/if-else in the step function |
| **3** | Full history in one Composite via ×ZERO stacking | The entire computation is one algebraic object |
| **4** | Division generates the same series as iteration | series_expand IS the loop — demonstrated on a counter |
| **5** | Busy beaver history = generating function | The BB2 computation corresponds to a series expansion |

---

## The Argument in One Paragraph

Laurent series expansion (`series_expand`) works by solving Q·D = N one coefficient at a time: each new coefficient depends on the previously computed ones and the denominator's structure. A Turing machine works by repeatedly reading the current state, applying a transition, and recursing on the new state. **These are the same algorithm.** The denominator encodes the program (transition function). The numerator encodes the initial state. Each quotient term is one computational step. The remainder reaching zero is halting. Since series expansion is a native operation of the composite algebra (it's just division), the execution loop is not external — it IS the algebra.

---

## Open Questions

1. **~~Can STEP be expressed as a single composite multiplication?~~** **ANSWERED.** STEP is a sum of 3 indicator-weighted products using only `+`, `×`, `read_dim`, `ZERO`, `INF`. See [STEP as Pure Composite Expression — Indicator Polynomial Test](https://www.notion.so/STEP-as-Pure-Composite-Expression-Indicator-Polynomial-Test-234cd4a92127404db4d86e099806d1ae?pvs=21).
2. **Does `series_expand(config₀, I - STEP·ZERO)` literally produce the history polynomial?** (Requires STEP as a linear operator — Stage 5 tests this structurally)
3. **Non-halting programs:** Division that doesn't terminate = series that doesn't converge = computation that doesn't halt. Is this correspondence exact?
4. **Self-reference:** Can the program (denominator) refer to its own quotient? If yes, that's the Y combinator in composite algebra.
